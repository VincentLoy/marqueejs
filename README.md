...
### start
Starts the scrolling (same as calling `resume`).

```typescript
const instance = marqueejs('.selector', {/* ...options... */})
instance.start()
```

### stop
Stops the scrolling and resets positions. If you call `start` afterward, all elements start from the beginning.
...
3. **Vertical Marquee**  
   ...
   marqueejs('.example-vertical', {
     ...
     contentList: [
       'Item One',
       'Item Two <b>bold text</b>',
       'Item Three with <i>italic text</i>'
     ]
   })
   ```
